import { StaffProfile, InventoryItem, LeaveRequest, DocumentArchive } from '../types';
import { Users, Briefcase, Calendar, FileText, ChevronRight, Activity, TrendingUp, AlertCircle, Clock } from 'lucide-react';
...
export default function Dashboard({
  ...
  const totalInventory = inventory.reduce((acc, curr) => acc + curr.quantity, 0);
  const itemsAssigned = inventory.filter((item) => item.status === 'Digunakan').reduce((acc, curr) => acc + curr.quantity, 0);
  const itemsDamaged = inventory.filter((item) => item.status === 'Rusak').reduce((acc, curr) => acc + curr.quantity, 0);

  const pendingLeaves = leaves.filter((l) => l.status === 'Pending');
  const totalLeavesApproved = leaves.filter((l) => l.status === 'Disetujui').length;

  const totalDocs = documents.length;

  // Let's create an elegant Indonesian activity log based on standard states
  const recentActivities = [
    {
      id: 1,
      type: 'leave',
      title: 'Pengajuan Cuti Baru',
      desc: `Dian Pratama mengajukan Cuti Tahunan untuk tanggal 28-30 Mei 2026.`,
      time: 'Baru saja',
    ...
    },
    {
      ...
      desc: 'Laptop ASUS Zenbook dialokasikan ke Senior Software Engineer Dian Pratama.',
    ...
    },
    {
      ...
      desc: 'Laporan_Keuangan_Q1_2026.pdf telah selesai diarsipkan oleh Keuangan.',
      time: 'Kemarin',
    ...
    },
    {
      id: 4,
      type: 'staff',
      title: 'Staf Bergabung',
      desc: 'Eka Wijaya resmi terdaftar sebagai staf General Affairs baru.',
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:auto-rows-[minmax(120px,auto)]">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-slate-300 max-w-xl text-sm leading-relaxed">
              Sistem Manajemen Aset Baznas.<br />
              <span className="font-semibold text-white/90">Aset Terdata, Amanah Terjaga.</span>
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 min-w-[140px] text-center shadow-sm">
              <span className="text-xs text-slate-300 font-medium uppercase tracking-wider block">Tahun Fiskal</span>
              <span className="text-2xl font-black font-mono tracking-wider block mt-1">2026</span>
            </div>
            <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 min-w-[140px] text-center shadow-sm">
              <span className="text-xs text-slate-300 font-medium uppercase tracking-wider block">Lokasi Utama</span>
              <span className="text-lg font-bold block mt-1">Jakarta, ID</span>
            </div>
          </div>
        </div>
      </div>
      ...
      >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Direktori Staf</span>
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="flex items-baseline space-x-2">
              <span className="text-4xl font-extrabold text-slate-900">{totalStaff}</span>
          ...
          <div className="mt-4 flex items-center justify-between text-xs border-t border-slate-100 pt-3 text-slate-500">
            <span className="font-medium">{activeStaff} Aktif • {staffOnLeave} Cuti</span>
      ...
      >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Inventaris Aset</span>
          ...
          <div className="mt-4 flex items-baseline space-x-2">
            <span className="text-4xl font-extrabold text-slate-900">{totalInventory}</span>
            <span className="text-slate-500 text-sm font-medium">Item</span>
          </div>
          <div className="mt-4 flex items-center justify-between text-xs border-t border-slate-100 pt-3 text-slate-500">
            <span className="font-medium">{itemsAssigned} Digunakan • {itemsDamaged} Rusak</span>
      ...
      >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Kontrol Cuti</span>
          ...
          <div className="mt-4 flex items-baseline space-x-2">
            <span className="text-4xl font-extrabold text-slate-900">{pendingLeaves.length}</span>
            <span className="text-slate-500 text-sm font-medium">Sertifikasi</span>
          </div>
          <div className="mt-4 flex items-center justify-between text-xs border-t border-slate-100 pt-3 text-slate-500">
            <span className="font-medium">{totalLeavesApproved} Cuti Disetujui</span>
      ...
      >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Arsip Dokumen</span>
          ...
          <div className="mt-4 flex items-baseline space-x-2">
            <span className="text-4xl font-extrabold text-slate-900">{totalDocs}</span>
      ...
      <div className="card-container p-6 space-y-4 md:col-span-2 lg:col-span-2 lg:row-span-3 flex flex-col">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="font-semibold text-sm text-slate-900 uppercase tracking-wider">Persetujuan Cuti</h3>
            </div>
          ...
          {pendingLeaves.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="mt-4 text-sm text-slate-400 font-medium">Semua permohonan cuti telah diproses.</p>
              <button 
                onClick={() => onNavigate('leave')}
              ...
              {pendingLeaves.map((req) => (
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center space-x-3">
                      <span className="font-bold text-sm text-slate-900">{req.staffName}</span>
                      <span className="text-[10px] font-bold tracking-wider bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded uppercase">
                        {req.leaveType}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 font-medium">
                      Tanggal: <span className="text-slate-800 font-semibold">{req.startDate} s/d {req.endDate}</span> ({req.durationDays} hari)
                    </p>
                    <p className="text-xs italic text-slate-400 max-w-sm truncate">"{req.reason}"</p>
                  </div>
        ...
        <div className="card-container p-6 space-y-4 lg:col-span-1 lg:row-span-3">
          <div className="flex items-center space-x-3 pb-4 border-b border-slate-100 shrink-0">
            ...
            <h3 className="font-semibold text-sm text-slate-900 uppercase tracking-wider">Aktivitas Terkini</h3>
          </div>
          ...
          <div className="space-y-4 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100">
            {recentActivities.map((act) => (
              <div key={act.id} className="flex space-x-4 relative">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 z-10 ${act.color} text-xs shadow-sm ring-4 ring-white`}>
                  {act.type === 'leave' && <Calendar className="w-5 h-5" />}
                  {act.type === 'inventory' && <Briefcase className="w-5 h-5" />}
                ...
                <div className="space-y-1 pt-1 overflow-hidden">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-bold text-slate-800 truncate">{act.title}</span>
                    <span className="text-[10px] text-slate-400 font-mono font-medium whitespace-nowrap bg-slate-50 px-2 py-0.5 rounded-full">{act.time}</span>
                  </div>
                  <p className="text-xs text-slate-500 leading-relaxed max-w-[280px]">{act.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card-container p-6 space-y-4 lg:col-span-1 lg:row-span-2 flex flex-col">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
            <h3 className="font-semibold text-sm text-slate-800 uppercase tracking-wider">Kondisi Inventaris</h3>
            <span className="text-xs font-bold text-slate-400 font-mono uppercase bg-slate-50 px-2.5 py-1 rounded-md">Rasio Distribusi</span>
          </div>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-600 font-medium font-sans">Bagus & Tersedia</span>
                <span className="text-slate-900 font-bold font-mono">
                  {inventory.length > 0 ? Math.round((inventory.filter(i => i.status === 'Tersedia').length / inventory.length) * 100) : 0}%
                </span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div 
                  className="bg-emerald-500 h-full rounded-full transition-all duration-1000 ease-out" 
                  style={{ width: `${inventory.length > 0 ? (inventory.filter(i => i.status === 'Tersedia').length / inventory.length) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-600 font-medium font-sans">Sedang Digunakan Staf</span>
                <span className="text-slate-900 font-bold font-mono">
                  {inventory.length > 0 ? Math.round((inventory.filter(i => i.status === 'Digunakan').length / inventory.length) * 100) : 0}%
                </span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div 
                  className="bg-indigo-500 h-full rounded-full transition-all duration-1000 ease-out delay-150" 
                  style={{ width: `${inventory.length > 0 ? (inventory.filter(i => i.status === 'Digunakan').length / inventory.length) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-600 font-medium font-sans">Rusak atau Masalah Fisik</span>
                <span className="text-slate-900 font-bold font-mono">
                  {inventory.length > 0 ? Math.round((inventory.filter(i => i.status === 'Rusak' || i.status === 'Dalam Perbaikan').length / inventory.length) * 100) : 0}%
                </span>
              </div>
              <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                <div 
                  className="bg-amber-500 h-full rounded-full transition-all duration-1000 ease-out delay-300" 
                  style={{ width: `${inventory.length > 0 ? (inventory.filter(i => i.status === 'Rusak' || i.status === 'Dalam Perbaikan').length / inventory.length) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-5 text-slate-100 space-y-4 flex flex-col justify-between shadow-xl relative overflow-hidden lg:col-span-1 lg:row-span-1 group hover:ring-2 hover:ring-emerald-500/30 transition-all cursor-default">
          ...
          <div className="space-y-3 relative z-10">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="font-semibold text-xs text-white uppercase tracking-wider">Panduan Cuti</h3>
            </div>
            <p className="text-xs text-slate-400 leading-relaxed">
              Maksimum 12-15 hari kerja per tahun. Rekam Nomor Seri di <strong className="text-slate-200">Inventaris</strong>. 
            </p>
          </div>
          ...
          <div className="p-3 bg-slate-800/50 rounded-xl flex items-start space-x-2 border border-slate-700/50 relative z-10 backdrop-blur-sm">
            ...
            <p className="text-[10px] text-slate-300 leading-relaxed">
              <strong className="text-white">Tips Arsip</strong>: Format didukung: .pdf, .docx, .xlsx.
            </p>
          </div>
        </div>
